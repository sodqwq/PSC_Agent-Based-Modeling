...
public class Agent implements Steppable {
	...
	public void replicate (Environment state, boolean gender){
		...
		Agent agent = new Agent(x, y, gender,attractiveness);
		agent.event = state.schedule.scheduleRepeating(agent);
		state.sparseSpace.setObjectLocation(agent,x, y);
		
		if(gender) {
			state.gui.setOvalPortrayal2DColor(agent, (float)1, (float)0, (float)0, (float)(attractiveness/state.maxAttractiveness));
		} else {
			state.gui.setOvalPortrayal2DColor(agent, (float)0, (float)0, (float)1, (float)(attractiveness/state.maxAttractiveness));
		}
	...
	public double frRule(Environment state) {
		...
		if (dates <= state.maxDates) {
			FR = ((state.maxDates+1) - (double)dates)/(state.maxDates);
		}
		return FR;//replace with appropriate code
	}
	...
	public double p4(Environment state, Agent a) {
		...
		return (fr*p1(state,a) + (1-fr)*p2(state,a) );
	}
